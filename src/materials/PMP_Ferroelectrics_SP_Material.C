...
#include "PMP_Ferroelectrics_SP_Material.h"

registerMooseObject("FerretApp",PMP_Ferroelectrics_SP_Material);

InputParameters PMP_Ferroelectrics_SP_Material::validParams()
{
    InputParameters params = Material::validParams();
    params.addClassDescription("DTU model using spontaneous polarization as order parameter");
    ...
    params.addRequiredParam<std::vector<Real>>("PermittivityCoe", "Permittivity coefficient");
    params.addRequiredParam<std::vector<Real>>("PiezoelectricityCoe", "Piezoelectric coefficient ");
    params.addRequiredParam<std::vector<Real>>("FlexoelectricityCoe", "Flexoelectricity coefficient ");
    params.addRequiredParam<std::vector<Real>>("GradientEnergyCoe", "Gradient energyCoe coefficient ");
...
PMP_Ferroelectrics_SP_Material::PMP_Ferroelectrics_SP_Material(const InputParameters & parameters)
    :Material(parameters),
    ...
    _PermittivityCoe(getParam<std::vector<Real>>("PermittivityCoe")),
    _PiezoelectricityCoe(getParam<std::vector<Real>>("PiezoelectricityCoe")),
    _FlexoelectricityCoe(getParam<std::vector<Real>>("FlexoelectricityCoe")),
    _GradientEnergyCoe(getParam<std::vector<Real>>("GradientEnergyCoe")),
    ...
    _e_const(declareProperty<RankThreeTensor>("e_const")),
    _e(declareProperty<RankThreeTensor>("e")),
    _E(declareProperty<RealVectorValue>("E")),
    _K(declareProperty<RankTwoTensor>("K")),
    _g(declareProperty<RankFourTensor>("g")),
    _flexo(declareProperty<RankFourTensor>("flexo")),
    _dedP(declareProperty<RankFourTensor>("dedP")),
    _d2NPdPdP(declareProperty<RankTwoTensor>("d2NPdPdP")),
    _vec_P(declareProperty<RealVectorValue>("vec_P")),
...
{
    RealVectorValue vec_1,vec_2;
    vec_1.zero();
    vec_1(0) = 1;
    vec_2.zero();
    vec_2(0) = vec_1(0)*cos(_theta/180*3.141592653) - vec_1(1)*sin(_theta/180*3.141592653);
    vec_2(1) = vec_1(0)*sin(_theta/180*3.141592653) + vec_1(1)*cos(_theta/180*3.141592653);

    RotG2L = RotationMatrix::rotVec1ToVec2(vec_1, vec_2);
    _K_Rot.zero();
    _K_Rot(0,0)=_PermittivityCoe[0];
    _K_Rot(1,1)=_PermittivityCoe[1];
    _K_Rot(2,2)=_PermittivityCoe[2];
    _K_Rot.rotate(RotG2L);
    ...
    //    b31 b31 b33 0   0   0];
    // b32=b31, d24=d15
    _PiezoCoe(2,0,0)=_PiezoelectricityCoe[0];  // b31
    _PiezoCoe(2,1,1)=_PiezoelectricityCoe[0];  // b32
    _PiezoCoe(2,2,2)=_PiezoelectricityCoe[2];  // b33
    _PiezoCoe(1,1,2)=_PiezoelectricityCoe[1];  // b24
    _PiezoCoe(1,2,1)=_PiezoelectricityCoe[1];  // b24
    _PiezoCoe(0,0,2)=_PiezoelectricityCoe[1];  // b15
    _PiezoCoe(0,2,0)=_PiezoelectricityCoe[1];  // b15
    _PiezoCoe_Rot.zero();
            ...
            for(unsigned int k=0;k<3;k++)
            {
                for(unsigned int l=0;l<3;l++)
                {
                    for(unsigned int m=0;m<3;m++)
                    {
                        ...
                        {
                            _PiezoCoe_Rot(i,j,k)=_PiezoCoe_Rot(i,j,k)+RotG2L(i,l)*RotG2L(j,m)*RotG2L(k,n)*_PiezoCoe(l,m,n);
                        }
                    }
                }
            }
    ...
    _gCoe.zero();
    // g(i,j,k,l) != g(j,i,k,l)
    _gCoe(0,0,0,0) = _GradientEnergyCoe[0];  // g11
    _gCoe(1,1,1,1) = _GradientEnergyCoe[0];  // g22
    _gCoe(2,2,2,2) = _GradientEnergyCoe[0];  // g33
    _gCoe(0,0,1,1) = _GradientEnergyCoe[1];  // g12
    _gCoe(1,1,0,0) = _GradientEnergyCoe[1];  // g21
    _gCoe(0,0,2,2) = _GradientEnergyCoe[1];  // g13
    _gCoe(2,2,0,0) = _GradientEnergyCoe[1];  // g31
    _gCoe(1,1,2,2) = _GradientEnergyCoe[1];  // g23
    _gCoe(2,2,1,1) = _GradientEnergyCoe[1];  // g32
    _gCoe(0,1,0,1) = _GradientEnergyCoe[2];  // g44
    _gCoe(1,0,1,0) = _GradientEnergyCoe[2];  // g44
    _gCoe(0,2,0,2) = _GradientEnergyCoe[2];  // g55
    _gCoe(2,0,2,0) = _GradientEnergyCoe[2];  // g55
    _gCoe(1,2,1,2) = _GradientEnergyCoe[2];  // g66
    _gCoe(2,1,2,1) = _GradientEnergyCoe[2];  // g66
    _gCoe.rotate(RotG2L);
    ...
    //    0     0     f44];
    // b32=b31, d24=d15
    _FlexoCoe(0,0,0,0)=_FlexoelectricityCoe[0];
    _FlexoCoe(0,0,0,0) = _FlexoelectricityCoe[0];  // f11
    _FlexoCoe(1,1,1,1) = _FlexoelectricityCoe[0];  // f22
    _FlexoCoe(2,2,2,2) = _FlexoelectricityCoe[0];  // f33
    _FlexoCoe(0,0,1,1) = _FlexoelectricityCoe[1];  // f12
    _FlexoCoe(1,1,0,0) = _FlexoelectricityCoe[1];  // f21
    _FlexoCoe(2,2,0,0) = _FlexoelectricityCoe[1];  // f13
    _FlexoCoe(0,0,2,2) = _FlexoelectricityCoe[1];  // f31
    _FlexoCoe(2,2,1,1) = _FlexoelectricityCoe[1];  // f32
    _FlexoCoe(1,1,2,2) = _FlexoelectricityCoe[1];  // f23
    _FlexoCoe(0,1,0,1) = 0.5*_FlexoelectricityCoe[2];  // f44
    _FlexoCoe(0,1,1,0) = 0.5*_FlexoelectricityCoe[2];  // f44
    _FlexoCoe(1,0,1,0) = 0.5*_FlexoelectricityCoe[2];  // f44
    _FlexoCoe(1,0,0,1) = 0.5*_FlexoelectricityCoe[2];  // f44
    _FlexoCoe(0,2,0,2) = 0.5*_FlexoelectricityCoe[2];  // f55
    _FlexoCoe(0,2,2,0) = 0.5*_FlexoelectricityCoe[2];  // f55
    _FlexoCoe(2,0,0,2) = 0.5*_FlexoelectricityCoe[2];  // f55
    _FlexoCoe(2,0,2,0) = 0.5*_FlexoelectricityCoe[2];  // f55
    _FlexoCoe(1,2,1,2) = 0.5*_FlexoelectricityCoe[2];  // f66
    _FlexoCoe(1,2,2,1) = 0.5*_FlexoelectricityCoe[2];  // f66
    _FlexoCoe(2,1,1,2) = 0.5*_FlexoelectricityCoe[2];  // f66
    _FlexoCoe(2,1,2,1) = 0.5*_FlexoelectricityCoe[2];  // f66
    _FlexoCoe.rotate(RotG2L);

}

void PMP_Ferroelectrics_SP_Material::computeQpProperties()
{
    ...
    _vec_P[_qp](0) = _couple_P1[_qp];
    _vec_P[_qp](1) = _couple_P2[_qp];
    _vec_P[_qp](2) = _couple_P3[_qp];
    dNormPdP = _vec_P[_qp]/NormP;
    for (unsigned int i = 0; i < _npolar; ++i)
    {
        for (unsigned int j = 0; j < _npolar; ++j)
        {
            d2NormPdPdP(i,j) = I(i,j)/NormP-_vec_P[_qp](i)*_vec_P[_qp](j)/(NormP*NormP*NormP);
        }
    }
    _d2NPdPdP[_qp] = d2NormPdPdP/_P0;
    // Permitivity and piezoelectric constants
    _K[_qp]       = _K_Rot;
    _e_const[_qp] = _PiezoCoe_Rot;
    _e[_qp]       = _e_const[_qp]*NormP/_P0;
    // Gradient energy coefficient
    _g[_qp]       = _gCoe;
    // Flexoelectricity constants
    _flexo[_qp]   = _FlexoCoe;
    ...
    // Derivatives of polarization dependent piezoelectric constants
    for (unsigned int i = 0; i < _npolar; ++i)
    {
        for (unsigned int j = 0; j < _npolar; ++j)
        {
            for (unsigned int k = 0; k < _npolar; ++k)
            {
                ...
                {
                    _dedP[_qp](i,j,k,l) = dNormPdP(l)*_e_const[_qp](i,j,k);
                }
            }
        }
    }
    ...
    for(unsigned int i = 0; i < _npolar; ++i)
    {
        _f_electric[_qp] = _f_electric[_qp] -_vec_P[_qp](i)*_E[_qp](i);
        for(unsigned int j = 0; j < _npolar; ++j)
        {
