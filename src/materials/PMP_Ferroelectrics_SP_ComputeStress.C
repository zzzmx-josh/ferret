...
#include "PMP_Ferroelectrics_SP_ComputeStress.h"

registerMooseObject("FerretApp",PMP_Ferroelectrics_SP_ComputeStress);

InputParameters PMP_Ferroelectrics_SP_ComputeStress::validParams()
{
...
PMP_Ferroelectrics_SP_ComputeStress::PMP_Ferroelectrics_SP_ComputeStress(const InputParameters &parameters)
...
void PMP_Ferroelectrics_SP_ComputeStress::computeQpStress()
{
    _stress[_qp].zero();
    ...
    for(unsigned int i = 0; i < _ndisp; ++i)
    {
        for(unsigned int j = 0; j < _ndisp; ++j)
        {
            ...
            for(unsigned int k = 0; k < _ndisp; ++k)
            {
                _f_elastic[_qp] = _f_elastic[_qp] -_e[_qp](i,j,k)*_mechanical_strain[_qp](i,j)*_E[_qp](k);
            }
        }
    }
