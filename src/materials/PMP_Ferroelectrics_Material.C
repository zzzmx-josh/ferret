...
#include "PMP_Ferroelectrics_Material.h"

registerMooseObject("FerretApp",PMP_Ferroelectrics_Material);

InputParameters PMP_Ferroelectrics_Material::validParams()
{
    InputParameters params = Material::validParams();
    params.addClassDescription("Define material properties, model using total polarization as order parameter");
    ...
    params.addRequiredParam<std::vector<Real>>("PermittivityCoe", "Permitivity");
    params.addRequiredParam<std::vector<Real>>("ElectrostrictiveCoe", "Electrostrictive coefficient ");
    params.addRequiredParam<std::vector<Real>>("FlexoelectricityCoe", "Flexoelectricity coefficient ");
    params.addRequiredParam<std::vector<Real>>("GradientEnergyCoe", "Gradient Energy coefficient ");
...
PMP_Ferroelectrics_Material::PMP_Ferroelectrics_Material(const InputParameters & parameters)
    :Material(parameters),
    ...
    _ElectrostrictiveCoe(getParam<std::vector<Real>>("ElectrostrictiveCoe")),
    ...
    _flexo_status(declareProperty<Real>("flexo_status"))
{
    _KCoe.zero();
    _KCoe(0,0)=_PermittivityCoe[0];
    _KCoe(1,1)=_PermittivityCoe[1];
    _KCoe(2,2)=_PermittivityCoe[2];

    // Electrostrictive constants
    _QCoe(0,0,0,0) = _ElectrostrictiveCoe[0];  // Q11
    _QCoe(1,1,1,1) = _ElectrostrictiveCoe[0];  // Q22
    _QCoe(2,2,2,2) = _ElectrostrictiveCoe[0];  // Q33
    _QCoe(0,0,1,1) = _ElectrostrictiveCoe[1];  // Q12
    _QCoe(1,1,0,0) = _ElectrostrictiveCoe[1];  // Q21
    _QCoe(0,0,2,2) = _ElectrostrictiveCoe[1];  // Q13
    _QCoe(2,2,0,0) = _ElectrostrictiveCoe[1];  // Q31
    _QCoe(1,1,2,2) = _ElectrostrictiveCoe[1];  // Q23
    _QCoe(2,2,1,1) = _ElectrostrictiveCoe[1];  // Q32
    _QCoe(0,1,0,1) = 0.5*_ElectrostrictiveCoe[2];  // Q44
    _QCoe(0,1,1,0) = 0.5*_ElectrostrictiveCoe[2];  // Q44
    _QCoe(1,0,1,0) = 0.5*_ElectrostrictiveCoe[2];  // Q44
    _QCoe(1,0,0,1) = 0.5*_ElectrostrictiveCoe[2];  // Q44
    _QCoe(0,2,0,2) = 0.5*_ElectrostrictiveCoe[2];  // Q55
    _QCoe(0,2,2,0) = 0.5*_ElectrostrictiveCoe[2];  // Q55
    _QCoe(2,0,0,2) = 0.5*_ElectrostrictiveCoe[2];  // Q55
    _QCoe(2,0,2,0) = 0.5*_ElectrostrictiveCoe[2];  // Q55
    _QCoe(1,2,1,2) = 0.5*_ElectrostrictiveCoe[2];  // Q66
    _QCoe(1,2,2,1) = 0.5*_ElectrostrictiveCoe[2];  // Q66
    _QCoe(2,1,1,2) = 0.5*_ElectrostrictiveCoe[2];  // Q66
    _QCoe(2,1,2,1) = 0.5*_ElectrostrictiveCoe[2];  // Q66
    ...
    _FlexoCoe(0,0,2,2) = _FlexoelectricityCoe[1];  // f13
    _FlexoCoe(2,2,0,0) = _FlexoelectricityCoe[1];  // f31
...
void PMP_Ferroelectrics_Material::computeQpProperties()
{
    // Absolute permittivity constants
    _K[_qp] = _KCoe;
    // Electrostrictive constants
    _Q[_qp] = _QCoe;
    ...
    // Check with or without flexoelectricity
    _flexo_status[_qp] = 0;
    if ( (_FlexoelectricityCoe[0]+_FlexoelectricityCoe[1]+_FlexoelectricityCoe[2])!=0 )
    {
        _flexo_status[_qp] = 1;
    }
    ...
    // Electric field
    RealVectorValue E0;
    E0(0) = _E00[_qp];                       // applied or eigen electric field
    E0(1) = _E01[_qp];
    E0(2) = _E02[_qp];
